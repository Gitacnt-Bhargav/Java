...
*/
    ...
    public static void main(String[] args) {
        designAStackWithIncrementOperation stk = new designAStackWithIncrementOperation(3);
        stk.push(1);                          // stack becomes [1]
        stk.push(2);                          // stack becomes [1, 2]
    ...
    public void increment(int k, int val) {
        k = Math.min(k, index+1); //because k is size or number of elements to be updated and index is the current length of that stack.
        //if k is higher than index+1, which means all elements are to be updated and not k
        //if index+1 is higher, than only k elements are to be updated
        for(int i=0; i<k;i++){
            stack[i] += val;
        }
    }

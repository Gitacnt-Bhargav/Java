...
public class asteroidCollisionPrgm {

//    Leet code - 735 - Medium
/*
...
*/
    ...
    public static void main(String[] args) {
        asteroidCollisionPrgm asteroidCollision = new asteroidCollisionPrgm();
    ...
    public int[] asteroidCollision(int[] asteroids) {
        ...
        for (int asteroid : asteroids) {
            while (!st.empty() && st.peek() > 0 && asteroid < 0 && st.peek() < -asteroid) {
                st.pop();
            }
            if (!st.empty() && st.peek() > 0 && asteroid < 0 ) {
                if(st.peek() == -asteroid)
                    st.pop();
            } else {
                st.push(asteroid);
